...
double width=2*1200.0;// window width in pixels
double height=2*700.0;// window height in pixels
...
double runge_kutta(int n){
    ...
    for(int i=0; i<n; i++){
        k1=h*f(i*h,yn);
        k2=h*f(i*h+0.5*h, yn+0.5*k1);
        k3=h*f(i*h+0.5*h, yn+0.5*k2);
        k4=h*f(i*h+h, yn+k3);
